...
### 6.10 Codex, 5ª rodada — no commit do MERGE (`545af27`), depois do merge — 1 P2 aceito por escrito

O "@codex review" pedido às 21:57 BRT de 19/09 foi respondido às 22:02, já sobre
o commit do merge (`545af27`, o merge das 21:56). O achado ficou sem resposta e
fora deste plano até a auditoria de 22/09/2026 — a linha "Codex no HEAD final"
do checklist fala do HEAD do PR, e esta rodada veio depois dele.
...
ANTES de `cb_assentar_mensagem_historica`, e a função soma 1 sempre que recebe
`p_conta_nao_lida = true`. Resposta de gente (CRM ou celular pareado) gravada no
vão entre as duas chamadas deixa a conversa com +1 de não lida sobre fala já
respondida, até alguém abri-la. A guarda da 1011 cobre só `aguardando_desde`.

**O que é verdade:** tudo acima. **Por que fica aceito, sem mudar código:**

- a janela é de UMA ida ao banco, e só no modo `historica`, que exige cópia sem
  telefone chegando DEPOIS de mensagens mais novas da mesma conversa;
- medido em 22/09/2026: desde o deploy, `cb_mensagens_sem_telefone` tem UMA
  ...
  como `nova` (a última da conversa). O modo `historica` ainda não rodou nenhuma
  vez em produção;
- o dano é o selo de não lida sobrando até a conversa ser aberta — nenhuma
  mensagem se perde, nenhum motor roda;
...
Se o modo `historica` começar a aparecer com frequência, a migration entra. A
medida é o log `modo: historica` da rota, ou, pelo banco, a recuperada
(`message_id` da tabela) que tem na mesma conversa outra mensagem de carimbo
MAIS NOVO gravada ANTES dela (`messages.gravada_em`, 1003).
...
- [x] Codex, 5ª rodada (commit do MERGE `545af27`, depois do merge): 1 P2 — a não lida da histórica decidida antes da função —, aceito por escrito em 22/09/2026 (6.10)
- [x] A primeira ocorrência real virou linha em `cb_mensagens_sem_telefone`: um documento de cliente em 21/09/2026 12:39 BRT, resolvido pelo acervo (medido em 22/09; a metade "log da VPS" do item abaixo continua em aberto)
