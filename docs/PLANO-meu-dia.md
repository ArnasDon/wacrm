...
## 16. Ajustes pedidos na tela (12/09/2026, depois da fase 2 em produção)

1. **O gesto que acorda a tela depois de 4 h também cancela a ação padrão** (Codex no PR #198): o Enter sobre um botão ou link focado ativava o clique sintetizado pelo navegador antes de o React desmontar o app. `keydown` e `pointerdown` passam a ser registrados sem `passive`, e o gesto que expira chama `preventDefault()`.
2. **Recorte por perfil nas novidades** (pedido do operador: "um advogado do trabalhista não precisa ter a tela poluída com notificações da conexão bancária"). Menções e conversas atribuídas carregam a conversa, então o aviso é recortado por `conversaNoEscopo`; avisos de TAREFA não são recortados, porque tarefa não tem conexão. O que fica fora aparece como "N fora do seu perfil" (régua da D8), nunca some — o sino não recorta, e sumir faria os dois números divergirem em silêncio. As conversas e a fila já eram recortadas desde a F1.
