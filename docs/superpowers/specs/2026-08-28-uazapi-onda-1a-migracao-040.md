...
| 1a-6 | `conversations.connection_id` e `broadcasts.connection_id` ficam **nullable** na 1a; `SET NOT NULL` + `ON DELETE RESTRICT` + ciclo de arquivo vão **juntos para a 1b/1c** | Achado na revisão da Task 1: `NOT NULL` obrigaria a tocar os paths de criação de conversa (inbound webhook, `resolve-conversation`) que são 1c; `NOT NULL` + `RESTRICT` quebra o botão "Reset Configuration" (`config/route.ts:441-465` faz `DELETE` da linha de config); e o RPC `create_broadcast_with_recipients` (038) insere em `broadcasts` sem `connection_id`. Com a coluna nullable + `ON DELETE SET NULL`, os três somem e o estado "órfão" pós-reset é o mesmo de hoje. **Custo (corrigido pela revisão final):** como NADA em `src/` popula `conversations.connection_id` na 1a (é 1c), TODA conversa criada após a 040 tem `connection_id` NULL — não só as de accounts que clicaram reset. Um índice único Postgres é `NULLS DISTINCT` por padrão, então `(acct, contact, NULL)` × `(acct, contact, NULL)` NÃO conflita e o backstop de conversa duplicada da migração 036 (bug real, changelog 0.8.1) some. **Mitigação (na 040):** o novo índice `idx_conversations_account_contact_connection` leva `NULLS NOT DISTINCT` — com todas as linhas NULL ele se comporta idêntico ao `(account_id, contact_id)` de 036; vira no-op quando a 1b/1c fizer `SET NOT NULL`. + asserção em `verify-schema.sql`. |
