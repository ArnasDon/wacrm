...
| 1ci-6 | Recorte-e-cola verbatim do seam | Ordenações sutis (o `conversation.created` antes da reação, a fronteira do `after()`) só sobrevivem sem reescrita; os ~20 testes de `webhook/route.test.ts` só são rede honesta se o código não mudar. |
