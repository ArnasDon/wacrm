...
function runSupabaseMigrationCheck(dbUrl) {
  ...
  return {
    ...
    message:
      has030 && has031
        ? 'Found migration versions 030 and 031 in remote migration history.'
        : 'Missing one or both required migration versions in output.',
  };
}

async function checkSavedZapiConfig(env) {
  const checks = [];
  ...
  checks.push({
    ...
    message: 'Z-API columns are present on whatsapp_config',
  });
  ...
  checks.push({
    ...
    message: 'at least one account has encrypted Z-API credentials and webhook secret',
  });
...
(async function main() {
  ...
  if (!zapiCheck.ok && zapiCheck.status === 'fail') allOk = false;
  ...
  if (migrationCheck.checked && !migrationCheck.ok) allOk = false;

  const savedConfigCheck = await checkSavedZapiConfig(env);
