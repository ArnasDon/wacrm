...
export default function PlatformAdminPage() {
  ...
  const markPaid = async (company: Company) => {
    if (!window.confirm(`¿Marcar ${company.name} como pagada y avanzar un mes su próximo pago?`)) return;
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "No se pudo marcar como pagada");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "No se pudo marcar como pagada");
    } finally {
  ...
  const changeDueDate = async (company: Company, value: string) => {
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "No se pudo cambiar la fecha de pago");
      await load();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "No se pudo cambiar la fecha de pago");
    } finally {
  ...
  const saveBankSettings = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "No se pudieron guardar los datos bancarios");
      setBankForm(body.settings);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "No se pudieron guardar los datos bancarios");
    } finally {
      setSavingBank(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Empresas</CardTitle><CardDescription>{companies.length} empresas registradas · consumo de los últimos 30 días</CardDescription></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Empresa</TableHead><TableHead>Dueño</TableHead><TableHead>Usuarios</TableHead><TableHead>Conversaciones</TableHead><TableHead>Mensajes</TableHead><TableHead>Tokens IA</TableHead><TableHead>Estado</TableHead><TableHead>Próximo pago</TableHead><TableHead>Alta</TableHead><TableHead className="text-right">Acción</TableHead></TableRow></TableHeader><TableBody>{companies.map((company) => <TableRow key={company.id}><TableCell className="font-medium">{company.name}</TableCell><TableCell><div>{company.owner?.name || "Sin nombre"}</div><div className="text-xs text-muted-foreground">{company.owner?.email || "Sin correo"}</div></TableCell><TableCell>{company.memberCount}</TableCell><TableCell>{company.usage30d.conversations.toLocaleString("es-GT")}</TableCell><TableCell>{company.usage30d.messages.toLocaleString("es-GT")}</TableCell><TableCell>{company.usage30d.aiTokens.toLocaleString("es-GT")}</TableCell><TableCell><span className={company.suspendedAt ? "text-destructive" : "text-emerald-500"}>{company.suspendedAt ? "Suspendida" : "Activa"}</span>{company.suspendedReason ? <div className="max-w-48 truncate text-xs text-muted-foreground" title={company.suspendedReason}>{company.suspendedReason}</div> : null}</TableCell><TableCell><Input type="date" defaultValue={toDateInputValue(company.nextPaymentDueAt)} disabled={savingDueDateId === company.id} onBlur={(event) => { if (event.target.value !== toDateInputValue(company.nextPaymentDueAt)) void changeDueDate(company, event.target.value); }} className="h-8 w-36 text-xs" />{company.lastMarkedPaidAt ? <div className="mt-1 text-xs text-muted-foreground">Último pago: {new Date(company.lastMarkedPaidAt).toLocaleDateString("es-GT")}</div> : null}</TableCell><TableCell>{new Date(company.createdAt).toLocaleDateString("es-GT")}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-2"><Button size="sm" variant="outline" disabled={markingPaidId === company.id} onClick={() => void markPaid(company)}>{markingPaidId === company.id ? "Guardando…" : "Marcar pagada"}</Button><Button size="sm" variant={company.suspendedAt ? "outline" : "destructive"} disabled={changingId === company.id} onClick={() => void changeSuspension(company)}>{changingId === company.id ? "Guardando…" : company.suspendedAt ? "Reactivar" : "Suspender"}</Button></div></TableCell></TableRow>)}{!loading && companies.length === 0 ? <TableRow><TableCell colSpan={10} className="py-8 text-center text-muted-foreground">No hay empresas registradas.</TableCell></TableRow> : null}</TableBody></Table></CardContent></Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Banknote className="size-5 text-primary" />Mis datos bancarios</CardTitle>
          <CardDescription>Se muestran a todas las empresas en Configuración → Facturación.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={saveBankSettings} className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label htmlFor="bankName">Banco</Label><Input id="bankName" value={bankForm.bank_name ?? ""} onChange={(event) => setBankForm((prev) => ({ ...prev, bank_name: event.target.value }))} /></div>
            <div className="space-y-2"><Label htmlFor="accountNumber">Número de cuenta</Label><Input id="accountNumber" value={bankForm.account_number ?? ""} onChange={(event) => setBankForm((prev) => ({ ...prev, account_number: event.target.value }))} /></div>
            <div className="space-y-2"><Label htmlFor="accountType">Tipo de cuenta</Label><Input id="accountType" value={bankForm.account_type ?? ""} onChange={(event) => setBankForm((prev) => ({ ...prev, account_type: event.target.value }))} /></div>
            <div className="space-y-2"><Label htmlFor="accountHolder">Titular</Label><Input id="accountHolder" value={bankForm.account_holder ?? ""} onChange={(event) => setBankForm((prev) => ({ ...prev, account_holder: event.target.value }))} /></div>
            <div className="sm:col-span-2"><Button type="submit" disabled={savingBank}>{savingBank ? "Guardando…" : "Guardar datos bancarios"}</Button></div>
          </form>
        </CardContent>
      </Card>
    </div>
