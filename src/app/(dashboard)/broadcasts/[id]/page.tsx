...
function FunnelChart({ steps }: { steps: FunnelStep[] }) {
  const t = useTranslations('broadcasts.detail.funnel');
  const max = Math.max(...steps.map((s) => s.value), 1);
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <h3 className="mb-4 text-sm font-medium text-foreground">{t('title')}</h3>
...
export default function BroadcastDetailPage() {
  ...
  function handleExport() {
    if (!broadcast) return;
    const header = [
      t('recipients.csvHeaders.contact'),
      t('recipients.csvHeaders.phone'),
      t('recipients.csvHeaders.status'),
      t('recipients.csvHeaders.sentAt'),
      t('recipients.csvHeaders.deliveredAt'),
      t('recipients.csvHeaders.readAt'),
      t('recipients.csvHeaders.repliedAt'),
      t('recipients.csvHeaders.error'),
    ];
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (delErr) {
      toast.error(t('delete.errorToast', { message: delErr.message }));
      return;
    }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
              <span>{t('templateLabel', { name: broadcast.template_name })}</span>
              <span>-</span>
              <span>
                {t('createdLabel', {
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-1.5 text-sm">
            <span className="text-red-300">{t('delete.confirmQuestion')}</span>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              {deleting ? t('delete.confirming') : tc('actions.confirm')}
            </Button>
          </div>
        ) : (
          <Button
            ...
            title={
              broadcast.status === 'sending'
                ? t('delete.tooltipDisabled')
                : t('delete.tooltipEnabled')
            }
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
          <h2 className="text-sm font-medium text-foreground">
            {statusFilter !== 'all'
              ? t('recipients.titleFiltered', {
                  count: filteredRecipients.length,
                  total: recipients.length,
                })
              : t('recipients.title', { count: filteredRecipients.length })}
          </h2>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-muted-foreground">{t('recipients.columns.contact')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.phone')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.status')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.sent')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.delivered')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.read')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('recipients.columns.error')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-border">
                      <TableCell className="font-medium text-foreground">
                        {recipient.contact?.name ?? t('recipients.unknownContact')}
                      </TableCell>
