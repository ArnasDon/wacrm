...
type Product = { id:string; name:string; description:string|null; price:number|string; currency:string; image_url:string|null; product_url:string|null; category:string|null; stock_quantity:number|null; is_active:boolean }
...
export default function CatalogPage() {
  ...
  async function saveAllBulk(){
    ...
    for(const item of ready){
      try{
        const description=item.color?`Cor: ${item.color}. ${item.description}`.trim():item.description
        const r=await fetch('/api/catalog/products',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:item.name,price:item.price,image_url:item.imageUrl,description})})
        const b=await r.json().catch(()=>({}))
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="products"><TabsList><TabsTrigger value="products">Produtos</TabsTrigger><TabsTrigger value="external">API externa{apiSources.length ? ` (${apiSources.length})` : ''}</TabsTrigger><TabsTrigger value="database">Base de dados{databaseSources.length ? ` (${databaseSources.length})` : ''}</TabsTrigger></TabsList>
        ...
        {bulkItems.length>0?<>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{bulkItems.map(item=><Card key={item.id}>
            ...
            <CardContent className="space-y-2 pt-4">
              ...
              {item.color?<p className="text-xs text-muted-foreground">Cor sugerida: <span className="font-medium text-foreground">{item.color}</span></p>:null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{products.length===0?<Card className="sm:col-span-2 xl:col-span-3"><CardContent className="py-10 text-center text-muted-foreground">Sem produtos.</CardContent></Card>:products.map(p=><Card key={p.id} className={!p.is_active?'opacity-60':''}>{p.image_url?<img src={p.image_url} alt={p.name} className="aspect-[4/3] w-full object-cover"/>:<div className="flex aspect-[4/3] items-center justify-center bg-muted"><ImageIcon className="h-10 w-10"/></div>}<CardHeader><CardTitle>{p.name}</CardTitle><CardDescription>{p.category||'Sem categoria'}</CardDescription></CardHeader><CardContent className="space-y-3"><p className="text-lg font-semibold">{Number(p.price).toLocaleString('pt-PT')} {p.currency}</p><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={()=>void toggleProduct(p)}>{p.is_active?'Desactivar':'Activar'}</Button><Button size="sm" variant="destructive" onClick={()=>void removeProduct(p.id)}><Trash2/>Remover</Button>{p.product_url?<Button size="sm" variant="ghost" render={<a href={p.product_url} target="_blank" rel="noreferrer"/>}><ExternalLink/>Abrir</Button>:null}</div></CardContent></Card>)}</div></TabsContent>
