...
export default function PipelinesPage() {
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const { error } = await supabase
        ...
        .eq("id", dealId);
      if (error) {
        toast.error(t("failedToMoveDeal"));
        refreshDeals();
      }
    },
  ...
  async function handleCreatePipeline() {
    ...
    // pipelines.account_id is NOT NULL post-017 with no DB default.
    if (!accountId) {
      toast.error(t("dealForm.profileNotLinked"));
      setCreating(false);
      return;
    }
    ...
    if (error || !pipeline) {
      toast.error(t("failedToCreatePipeline"));
      setCreating(false);
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Dialog open={newPipelineOpen} onOpenChange={setNewPipelineOpen}>
        <DialogContent className="sm:max-w-sm bg-popover border-border">
          ...
          <div className="py-2">
            <Label className="text-muted-foreground">{t("settings.pipelineNameLabel")}</Label>
            <Input
          ...
          <DialogFooter className="bg-popover/50 border-border">
            ...
            <Button
              ...
              className="bg-primary text-primary-foreground hover:bg-primary/90"
            >
              <span>{creating ? t("creating") : t("addPipeline")}</span>
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
