...
// Automations are account-shared (RLS: `is_account_member(account_id)` for
// read, `…, 'agent'` for write). The service-role client below bypasses
// RLS, so every handler scopes explicitly by `account_id` — matching how
// the sibling `flows/[id]` route already works. Scoping by `user_id`
// (the old single-user model) wrongly 404'd teammates on automations they
// didn't personally create, even though the list view showed them.
...
export async function PATCH(
  ...
  // Account ownership check before we touch anything. Load the fields we
