...
export async function POST(req: NextRequest) {
  ...
  const { event_id, event_type, attribution, payload, ref_code, landing_slug } = parsed.data;
  ...
  if (event_type === "form_submit") {
    ...
    if (!phone) {
      return NextResponse.json({ error: "'payload.phone' is required for form_submit" }, { status: 400 });
    }
