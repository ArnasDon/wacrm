    ...
    for (const change of entry.changes) {
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        await processMessage(
          ...
          // read before migration 061 lands would have it undefined,
...
async function processMessage(
  ...
  accessToken: string,
  // Per-account opt-out for the inbound-media mirror (migration 061).
  ...
  // unique index on (conversation_id, message_id) added in migration 059
    ...
    .upsert(
      {
        ...
        media_url: mediaUrl,
        // Meta's MIME type for the attachment (migration 061). Was
  ...
  // Update conversation. The unread bump is done DB-side (migration 059's
