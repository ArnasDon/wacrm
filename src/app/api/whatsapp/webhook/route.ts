...
} from '@/lib/whatsapp/find-or-create'
...
// handleStatusUpdate / flagBroadcastReplyIfAny / lookupInternalIdByMetaId /
// handleReaction moved verbatim to src/lib/whatsapp/inbound-events.ts so
// DemoWhatsAppService can drive the exact same status/reaction handling
// for a simulated event as this route does for a real one (§20: demo
// mode writes into the same tables production analytics reads).
...
// findOrCreateContact / findOrCreateConversation moved verbatim to
// src/lib/whatsapp/find-or-create.ts so a simulated demo inbound
// message can resolve the same contact/conversation a real inbound
// webhook would, rather than a second hand-rolled lookup.
