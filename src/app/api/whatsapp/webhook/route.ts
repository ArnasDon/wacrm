  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      const accounts = new Set(
        configRows.map((r: { account_id: string }) => r.account_id)
      );
      if (accounts.size > 1) {
        console.error(
          `Multiple account owners (${accounts.size}) found for phone_number_id:`,
          phoneNumberId,
          '— inbound message dropped. Resolve cross-account number collision.',
          'Account owners:',
          configRows.map(
