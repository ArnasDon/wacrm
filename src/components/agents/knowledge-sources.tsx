...
export function KnowledgeSources() {
  const canEdit = useCan('manage-settings')
  ...
  const testConnection = async () => {
    ...
    setTesting(true)
    try {
      const response = await fetch('/api/ai/knowledge-sources/novyra/test', {
        method: 'POST',
      })
      const data = await response.json().catch(() => ({}))
      setState((current) =>
    ...
    <div className="max-w-3xl space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Escolha, por agente, as fontes externas que podem acrescentar informação ao contexto da IA.
        </p>
      </div>
          ...
          <div className="flex gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <CardTitle>NOVURA</CardTitle>
                <Badge variant={state.enabled && state.configured ? 'default' : 'secondary'}>
                  {statusLabel}
                </Badge>
              </div>
              <CardDescription className="mt-2 max-w-xl">
                Base de conhecimento de notícias e informação verificada sobre Moçambique.
              </CardDescription>
            </div>
          </div>
          <Switch
            ...
            aria-label="Activar ou desactivar a NOVURA para este agente"
          />
        </CardHeader>
        ...
        <CardContent className="space-y-5">
          {!state.configured && (
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">
              Configuração do servidor incompleta. Confirme as variáveis NOVYRA no servidor antes de activar esta fonte.
            </div>
          )}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-border p-3">
              ...
              <p className="mt-2 text-sm font-medium text-foreground">Moçambique</p>
            </div>
            <div className="rounded-lg border border-border p-3">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Código do agente</p>
              <code className="mt-2 block break-all text-sm text-foreground">
                {state.configuration.agentCode}
              </code>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 rounded-lg border border-border p-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {state.last_tested_at
                  ? new Intl.DateTimeFormat('pt-PT', {
                      dateStyle: 'medium',
                      timeStyle: 'short',
                    }).format(new Date(state.last_tested_at))
                  : 'Ainda não realizado'}
            ...
            <Button
              type="button"
              variant="outline"
              disabled={!canEdit || !state.configured || !state.agent_id || testing}
