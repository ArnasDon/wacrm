...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  update_contact_field: { label: "Atualizar Campo de Contato", icon: PencilLine, border: "border-l-primary" },
  create_deal: { label: "Criar Negócio", icon: Briefcase, border: "border-l-primary" },
  wait: { label: "Aguardar", icon: Hourglass, border: "border-l-slate-500" },
        ...
        <>
          <FieldBlock label="Modo">
            ...
            >
              ...
              <option value="specific">Atendente específico</option>
            </select>
          </FieldBlock>
          {cfg.mode === "specific" && (
            <FieldBlock label="ID do Atendente">
              <Input
        ...
        <>
          <FieldBlock label="Assunto">
            ...
            >
              ...
              <option value="contact_field">Campo de contato</option>
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "wait":
      const unit = step.step_config.unit === "minutes" ? "minutos" : step.step_config.unit === "hours" ? "horas" : step.step_config.unit === "days" ? "dias" : step.step_config.unit;
      return `${step.step_config.amount ?? "?"} ${unit ?? ""}`
    case "condition":
      const subject = step.step_config.subject === "tag_presence" ? "presença de tag" : step.step_config.subject === "contact_field" ? "campo de contato" : step.step_config.subject === "message_content" ? "conteúdo da mensagem" : step.step_config.subject === "time_of_day" ? "hora do dia" : step.step_config.subject;
      return `quando ${subject ?? "?"}`
    case "send_webhook":
