...
export function ImportModal({
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!accountId)
        throw new Error(t('toasts.profileNotLinked'));
      ...
      try {
        ...
        toast.warning(t('toasts.someTagAssignmentsFailed'));
      }

      setResult({ imported, skipped, failed, tagsAssigned });
      if (imported > 0) {
        toast.success(t('toasts.contactsImported', { count: imported }));
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(
          t('toasts.tagAssignmentsApplied', { count: tagsAssigned })
        );
      }
      if (skippedNames.length > 0) {
        ...
        const more =
          skippedNames.length > 3
            ? t('toasts.moreTagsSuffix', { count: skippedNames.length - 3 })
            : '';
        toast.info(
          `${t('toasts.unknownTagsSkipped', { sample })}${more}`
        );
      }
      if (skipped > 0) {
      ...
      if (failed > 0) {
        toast.error(t('toasts.contactsFailedToImport', { count: failed }));
      }
    } catch (err: unknown) {
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] font-semibold tracking-[0.14em] text-muted-foreground uppercase">
                  {t('previewLabel', { count: preview.length })}
                </p>
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-md bg-muted/90 px-2 py-0.5 text-[11px] text-muted-foreground">
                      ...
                      {t('tagsStat', {
                        tagCount: tagStats.unique,
                        rowCount: tagStats.rowsWithTags,
                      })}
                    </span>
                  )}
                </div>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  {t('moreRowsNotShown', { count: parsedRows.length - PREVIEW_LIMIT })}
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    <CheckCircle className="size-4 shrink-0" />
                    {t('tagsAssignedCount', { count: result.tagsAssigned })}
                  </div>
                )}
                {result.skipped > 0 && (
