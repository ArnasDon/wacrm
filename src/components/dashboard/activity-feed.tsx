          ...
          <footer className="flex items-center justify-between border-t border-border px-5 py-3 text-xs">
            <span className="text-muted-foreground tabular-nums">
              {t('activityFeed.showing', {
                ...
                total: totalLoaded === 50 ? `${totalLoaded}+` : totalLoaded,
              })}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-muted-foreground">{t('activityFeed.show')}</span>
...
function relativeTime(iso: string, t: ReturnType<typeof useTranslations>): string {
  ...
  if (diffSec < 60) return t('activityFeed.secondsAgo', { n: Math.max(1, diffSec) })
  if (diffSec < 3600) return t('activityFeed.minutesAgo', { n: Math.floor(diffSec / 60) })
  if (diffSec < 86400) return t('activityFeed.hoursAgo', { n: Math.floor(diffSec / 3600) })
  if (diffSec < 2_592_000) return t('activityFeed.daysAgo', { n: Math.floor(diffSec / 86400) })
