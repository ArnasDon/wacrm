...
import { getEmailCampaignStatus } from '@/lib/email-campaign-status';
...
/**
 * Tabla de campañas de email con polling mientras alguna esté en envío.
 * Espejo de la lista de broadcasts (page.tsx de /broadcasts) pero sobre
 * `email_campaigns` y con la tasa de apertura en vez de "read".
 */
export function EmailCampaignsList() {
  ...
  // Fetch on mount with the fetch inline in the effect — mirrors
  // broadcasts/[id] so the React Compiler keeps its setState-in-effect
  // analysis happy (external async fns that setState are flagged).
  useEffect(() => {
  ...
  useEffect(() => {
    function startPolling() {
      ...
      pollTimer.current = setInterval(refreshCampaigns, POLL_INTERVAL_MS);
    }
  ...
  return (
    <div className="space-y-6">
      {anySending && (
        <div
          role="progressbar"
          aria-label="Email campaign in progress"
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border hover:bg-transparent">
                ...
                <TableHead className="hidden text-muted-foreground lg:table-cell">{t('table.open')}</TableHead>
