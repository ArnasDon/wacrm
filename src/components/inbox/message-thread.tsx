...
function formatDateSeparator(
  ...
  if (isToday(date)) return t("dateToday");
  if (isYesterday(date)) return t("dateYesterday");
...
const STATUS_VALUES: { value: ConversationStatus; color: string }[] = [
...
export function MessageThread({
  ...
  const tMessageBubble = useTranslations("inbox.messageBubble");
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg) return { expired: true, remaining: t("session.noCustomerMessages") };

    const hoursSince = differenceInHours(new Date(), new Date(lastCustomerMsg.created_at));
    const expired = hoursSince >= 24;

    if (expired) {
      return { expired: true, remaining: t("session.expired") };
    }

    const hoursLeft = 24 - hoursSince;
    const remaining =
      hoursLeft >= 1
        ? t("session.hoursRemaining", { hours: Math.floor(hoursLeft) })
        : t("session.minutesRemaining", { minutes: Math.floor(hoursLeft * 60) });

    return { expired, remaining };
  }, [messages, isOfficialProvider, t]);
  ...
  const handleSendMedia = useCallback(
    async (payload: SendMediaPayload) => {
      ...
      const contentText =
        payload.kind === "document"
          ? payload.caption || payload.filename || tMessageBubble("document")
          : payload.caption;
    ...
    [conversation, onNewMessage, onUpdateMessage, t, tMessageBubble],
  );
  ...
  const handleStartReply = useCallback(
    (msg: Message) => {
      setReplyTo({
        ...
        preview: buildReplyPreview(msg, tReplyQuote),
      });
    },
    [authorLabelFor, tReplyQuote],
  );
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error(t("toasts.waitForSend"));
        return;
      }
  ...
  const handleAssignChange = useCallback(
    async (agentId: string | null) => {
      ...
      if (error) {
        ...
        toast.error(t("toasts.assignmentFailed"));
        return;
      }
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                ...
                {currentStatus?.label ?? t("statusFallback")}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-muted-foreground">{t("noMessages.title")}</p>
                  ...
                  {group.messages.map((msg) => {
                    ...
                    const reply = parent
                      ? {
                          ...
                          preview: buildReplyPreview(parent, tReplyQuote),
                        }
                      : null;
