...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    // "lost"/"won" -> "LOST"/"CONVERTED" per migration 055's status
    // widening (deals extended into Lead, §9.0/§23 Phase 6).
    const active = deals.filter((d) => d.status !== 'LOST');
    const openDeals = active.filter((d) => d.status !== 'CONVERTED');
    ...
    const wonThisMonth = deals.filter(
      (d) => d.status === 'CONVERTED' && thisMonth(d)
    ).length;
