...
export function PipelineSettings({
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("toasts.stageInUse"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("toasts.deletePipelineSuccess"));
  }
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? tCommon("actions.deleting") : t("deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("pickColorAria", { color })}
                    />
                  ))}
                </div>
