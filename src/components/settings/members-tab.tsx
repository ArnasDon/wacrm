...
// The value/order is fixed here; labels + hints are resolved from
// translations inside the component (module scope can't call `t`).
...
export function MembersTab() {
  ...
  const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] =
    EDITABLE_ROLE_VALUES.map((value) => ({
      value,
      label: tRoles(value),
      hint: t(`editableRoleHints.${value}`),
    }));
  ...
  function fmtExpiresIn(iso: string): string {
    ...
    if (days >= 1) return t('expiry.expiresInDays', { n: days });
    const hours = Math.max(1, Math.floor(ms / (60 * 60 * 1000)));
    return t('expiry.expiresInHours', { n: hours });
  }
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('errors.loadMembersFailed'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('errors.loadInvitationsFailed'));
          return;
        }
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('errors.updateRoleFailed'));
        return;
      }
      toast.success(
        t('toasts.roleUpdated', {
          name: member.full_name || t('unnamedMember'),
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('errors.removeMemberFailed'));
        return;
      }
      toast.success(
        t('toasts.memberRemoved', {
          name: removingMember.full_name || t('unnamedMember'),
        }),
      );
      setMembers((prev) =>
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('errors.revokeInvitationFailed'));
        return;
      }
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                ...
                {t('presence.online', { n: counts.online })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="away" />
                {t('presence.away', { n: counts.away })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="offline" />
                {t('presence.offline', { n: counts.offline })}
              </span>
              ...
              return (
                ...
                >
                          ...
                          <Avatar className="size-9 shrink-0">
                            {member.avatar_url ? (
                              <AvatarImage
                                ...
                                alt={member.full_name || t('unnamedMember')}
                              />
                            ) : null}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="truncate text-sm font-medium text-foreground">
                          {member.full_name || t('unnamedMember')}
                        </span>
                        {isSelf && (
                  ...
                  <div className="hidden sm:block text-right text-xs text-muted-foreground">
                    {t('joinedOn', { date: fmtDate(member.joined_at) })}
                  </div>
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              {t('pendingInvitations.noResendHint')}
            </p>
          ) : null}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t.rich('pendingInvitations.emptyHint', {
                    highlight: (chunks) => (
                  ...
                  {invitations.map((inv) => {
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-foreground">
                            {inv.label || t('pendingInvitations.untitled')}
                          </span>
                          <span
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {t('pendingInvitations.createdMeta', {
                            date: fmtDate(inv.created_at),
                            expiry: fmtExpiresIn(inv.expires_at),
                          })}
                        </p>
                      </div>
                      ...
                      >
                        ...
                        {t('pendingInvitations.revokeButton')}
                      </Button>
                    </li>
                    );
                  })}
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {t.rich('removeDialog.description', {
                name: removingMember?.full_name || t('removeDialog.fallbackName'),
                strong: (chunks) => (
