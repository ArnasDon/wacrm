...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t('avatar.unsupportedType'), {
        description: t('avatar.allowedFormats'),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pendingAvatar) {
        ...
        if (uploadError) {
          throw new Error(t('errors.uploadFailed', { message: uploadError.message }));
        }
        const {
          data: { publicUrl },
      ...
      const { error: updateError } = await supabase
        ...
        .eq('user_id', user.id);
      if (updateError) {
        throw new Error(t('errors.saveFailed', { message: updateError.message }));
      }
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.error(t('errors.emailChangeFailed', { message: emailError.message }));
          setSaving(false);
      ...
      toast.success(
        emailSent ? t('toasts.profileSavedEmailPending') : t('toasts.profileSaved'),
      );
    } catch (err) {
        ...
        <Card>
          ...
          <div className="flex flex-wrap items-center gap-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {currentAvatar ? t('avatar.changePhoto') : t('avatar.uploadPhoto')}
              </Button>
              {currentAvatar && (
                <Button
                  type="button"
          ...
          <div className="rounded-lg border border-border bg-muted p-4">
            ...
            <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-muted-foreground">{t('accountDetails.role')}</dt>
                <dd className="mt-0.5 font-mono text-foreground">
                  {profile?.role ?? 'user'}
                </dd>
              </div>
              <div>
                <dt className="text-muted-foreground">{t('accountDetails.joined')}</dt>
                <dd className="mt-0.5 text-foreground">{joined}</dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-muted-foreground">{t('accountDetails.userId')}</dt>
