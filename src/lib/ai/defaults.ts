  ...
  if (mode === 'auto_reply') {
    ...
    if (dealStageOptions && dealStageOptions.otherStageNames.length > 0) {
      if (dealStageOptions.hasDeal) {
        parts.push(
          `This contact has an open deal currently at the "${dealStageOptions.currentStageName}" stage. If — and only if — the conversation itself clearly shows the deal has moved forward to one of these other stages: ${dealStageOptions.otherStageNames.map((n) => `"${n}"`).join(', ')}, append ${MOVE_DEAL_SENTINEL_PREFIX}<exact stage name>${MOVE_DEAL_SENTINEL_SUFFIX} at the very end of your reply (after your customer-facing message, and after the purchase-confirmation marker above if both apply), using the exact stage name as written above — never a name outside this list, never the deal's current stage, and never a guess when the signal is ambiguous. This is for ordinary progress (e.g. the customer asked for a quote, or is now negotiating terms) — it is separate from, and does not replace, the purchase-confirmation marker above: consider this marker even on a turn where you're also asking for final purchase confirmation and NOT yet using the purchase-confirmation marker. Never mention this marker to the customer.`,
        )
      } else {
        parts.push(
          `This contact does not have a deal yet. If — and only if — the conversation clearly shows real buying interest (they're asking about pricing to buy, want next steps to purchase, said things like "I want it" / "I'll take it" / "I'm very interested", are negotiating, etc. — not idle chit-chat or a vague "maybe later"), append ${MOVE_DEAL_SENTINEL_PREFIX}<exact stage name>${MOVE_DEAL_SENTINEL_SUFFIX} at the very end of your reply, using one of these exact stage names: ${dealStageOptions.otherStageNames.map((n) => `"${n}"`).join(', ')}. This creates a new deal directly at that stage — pick the stage that best matches how far along they already are, never a name outside this list, and never use it just because the customer said hello. Consider this marker even on a turn where you're also asking for final purchase confirmation and NOT yet using the purchase-confirmation marker above — strong interest should land the contact in the pipeline right away, it doesn't need to wait until the sale is fully closed. Never mention this marker to the customer.`,
        )
      }
    }
