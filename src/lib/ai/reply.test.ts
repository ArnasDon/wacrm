...
vi.mock('./admin-client', () => {
  ...
  function resolve(ops: {
    ...
    if (table === 'contacts') {
      return {
        data: state.contactPhone ? { phone: state.contactPhone } : null,
        error: null,
      };
    }
    ...
    if (table === 'messages') {
      // Honour loadHistory's .neq('id', excludeMessageId) so tests can
      // assert the triggering inbound row is excluded from history.
      const rows = ops.neqId
        ? state.history.filter((m) => m.id !== ops.neqId)
        : state.history;
  ...
  function builder(table: string) {
    ...
    const b: Record<string, unknown> = {
      ...
      eq: () => b,
      neq: (_col: unknown, val: unknown) => ((ops.neqId = val), b),
      gte: () => b,
      order: () => b,
...
const sendAiReply = vi.fn(async (_args: unknown) => ({
...
describe('maybeReplyToInbound — happy path → send + log', () => {
  ...
  it('passes the assembled system + messages to the model wrapper', async () => {
    ...
    expect(arg.system[arg.system.length - 1].text).toContain(
      'Open 9-5 Mon-Fri.'
    );
  ...
  it('excludes the just-inserted inbound row from history (no duplicate user turn)', async () => {
    // The webhook stores the inbound message BEFORE dispatching us, so it
    // is already in the conversation history under the same id we pass as
    // messageId. loadHistory must drop it — otherwise buildMessages would
    // append the inbound a second time, producing two consecutive `user`
    // turns (an Anthropic 400 that silently forces escalation on every
    // message). Regression for the code-review finding.
    h.state.history = [
      {
        id: 'msg-0',
        sender_type: 'bot',
      ...
      {
        id: 'msg-1', // === BASE_ARGS.messageId — the row just inserted
        sender_type: 'customer',
        content_text: 'what are your opening hours?',
    ...
    // The inbound appears exactly once (the final user turn); the prior
    // bot turn maps to assistant, so roles alternate cleanly.
    const inboundTurns = arg.messages.filter(
      (m) => m.content === 'what are your opening hours?'
    );
