...
async function loadHistory(
  ...
  const { data, error } = await db
    ...
    // Exclude the inbound row the webhook just inserted before dispatching
    // us. buildMessages() appends the inbound text as the final `user`
    // turn, so leaving it in history too would duplicate it — two
    // consecutive `user` turns, which the Anthropic API rejects (400),
    // silently routing every message to the catch → escalation path and
    // meaning the assistant would NEVER actually reply.
...
export async function maybeReplyToInbound(
  ...
  try {
    ...
    const { data: conversation } = await db
      ...
      // Defence in depth: the webhook resolves conversationId from the
      // account-scoped whatsapp_config lookup, but scope by account here
      // too so a future caller can never drive the engine across tenants.
      .eq('account_id', accountId)
      .maybeSingle();
    if (
      conversation &&
      (conversation as { ai_handling?: boolean }).ai_handling === false
    ) {
      await logAiReply({
        accountId,
        conversationId,
    ...
    const escalate = (
      reason: 'keyword' | 'cap_reached' | 'low_confidence' | 'error'
    ) =>
    ...
    const [kbEntries, history] = await Promise.all([
      ...
      loadHistory(conversationId, messageId),
    ]);
