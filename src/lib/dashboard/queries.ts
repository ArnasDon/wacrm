...
// `deals.status` was widened by migration 055 (§9.0/§23 Phase 6:
// extended into `Lead`) from 'open'|'won'|'lost' to an 8-value
// funnel enum — old rows are remapped, not left with 'open'. "Open
// pipeline value" now means "not yet CONVERTED or LOST", i.e. every
// non-terminal status, rather than a single literal value.
...
export async function loadMetrics(db: DB): Promise<MetricsBundle> {
  ...
  const [
    ...
    db
      ...
      .gte('created_at', yesterdayStart)
      .lt('created_at', todayStart),
    db.from('deals').select('value, status').in('status', OPEN_DEAL_STATUSES),
    db
      .from('messages')
      .select('id', { count: 'exact', head: true })
