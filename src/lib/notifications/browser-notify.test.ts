...
describe('shouldNotifyForMessage', () => {
  ...
  it('dedupes a replayed INSERT for the same id inside the window', () => {
    ...
    expect(shouldNotifyForMessage(customerMsg(), { ...opts, now: NOW })).toBe(
      true
    );
    expect(
    ...
    expect(
      shouldNotifyForMessage(customerMsg({ id: 'm2' }), {
        ...opts,
        now: NOW + 1_000,
      })
    ).toBe(true);
...
describe('pickContactDisplayName', () => {
  it('prefers name, then @username, then phone', () => {
    ...
    expect(
      pickContactDisplayName({ name: '', wa_username: 'ada', phone: '+1' })
    ).toBe('@ada');
    expect(
      pickContactDisplayName({ name: null, wa_username: null, phone: '+1' })
    ).toBe('+1');
  });
  ...
  it('returns null when nothing is usable', () => {
    ...
    expect(
      pickContactDisplayName({ name: ' ', wa_username: '', phone: '' })
    ).toBeNull();
  });
});
...
describe('viewedConversationFromLocation', () => {
  it('reads ?c= only on the inbox route', () => {
    ...
    expect(
      viewedConversationFromLocation('/inbox-archive', '?c=abc')
    ).toBeNull();
  });
});
