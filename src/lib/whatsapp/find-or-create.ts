// ============================================================
// Contact/conversation find-or-create — extracted verbatim from the
// webhook route (`findOrCreateContact` / `findOrCreateConversation`
// used to live there) so a simulated demo inbound message
// (`simulateDemoInboundMessage`, demo-simulate.ts) can resolve the
// exact same contact/conversation a real inbound webhook would,
// instead of a second hand-rolled lookup that could drift from it.
//
// No behaviour change from the original webhook-route versions —
// a pure move, save for swapping the route's local lazy admin client
// for the shared one every other lib file in this directory already
// uses (functionally identical: both are just
