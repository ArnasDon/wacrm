...
function makeDb(script: Script): SupabaseClient {
  ...
  const builder: Record<string, unknown> = {
    ...
    order: () => builder,
    limit: () => {
      // Only the conversation lookup terminates on `.limit(1)` directly
      // (no `.maybeSingle()` after it). resolveWhatsAppConfig's
      // "no explicit id, no default marked" fallback DOES chain
      // `.limit(1).maybeSingle()`, so whatsapp_config must stay
      // chainable here rather than resolving early.
