...
describe('resolveInboundIdentity', () => {
  ...
  it('normalizes the phone the way the DB does', () => {
    expect(resolveInboundIdentity({ from: '+1 (555) 123-0000' }).phone).toBe(
      '15551230000'
    );
  });
...
describe('hasUsableIdentity', () => {
  ...
  it('is false when Meta sent neither', () => {
    expect(
      hasUsableIdentity(
        resolveInboundIdentity({}, { profile: { name: 'Ada' } })
      )
    ).toBe(false);
  });
});
...
describe('identityDisplayName', () => {
  it('walks name → username → phone → BSUID', () => {
    ...
    expect(
      identityDisplayName(resolveInboundIdentity({ from: '15551230000' }))
    ).toBe('15551230000');
...
describe('contactHandle', () => {
  ...
  it('shows @username for a contact with no phone', () => {
    expect(contactHandle({ phone: '', wa_username: 'realsheenanelson' })).toBe(
      '@realsheenanelson'
    );
  });
  ...
  it('is empty only when the contact carries no identity at all', () => {
    ...
    expect(
      contactHandle({ phone: null, wa_username: null, wa_user_id: null })
    ).toBe('');
  });
});
