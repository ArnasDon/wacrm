...
describe('describeWabaPhoneMismatch', () => {
  it("names both ids and lists the WABA's numbers", () => {
    ...
    expect(text).toMatch(
      /Phone Number ID 333 does not belong to WhatsApp Business Account 999/
    );
  ...
  it('says so when the WABA has no numbers at all', () => {
    expect(describeWabaPhoneMismatch([], '333', '999')).toMatch(
      /lists no phone numbers/
    );
  });
...
describe('appSubscriptionState', () => {
  ...
  it('reports subscribed + match when META_APP_ID is listed', () => {
    expect(appSubscriptionState(subs, 'app-1')).toEqual({
      subscribed: true,
      appIdMatch: true,
    });
  });
  ...
  it('reports subscribed but no match when a different app holds the WABA', () => {
    expect(appSubscriptionState(subs, 'app-9')).toEqual({
      subscribed: true,
      appIdMatch: false,
    });
  });
  ...
  it('leaves the match unknown when META_APP_ID is not configured', () => {
    expect(appSubscriptionState(subs, undefined)).toEqual({
      subscribed: true,
      appIdMatch: null,
    });
    expect(appSubscriptionState(subs, '  ')).toEqual({
      subscribed: true,
      appIdMatch: null,
    });
  });
  ...
  it('reports not subscribed for an empty list', () => {
    expect(appSubscriptionState([], 'app-1')).toEqual({
      subscribed: false,
      appIdMatch: false,
    });
  });
