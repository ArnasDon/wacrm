-- ============================================================
-- 1019 — `stage_changed` passa a EXIGIR `to_pipeline_id`.
--
-- Achado do Codex no PR #232, sobre a 1014 e ainda vivo na 1017: a conferência
-- de forma aceitava um `stage_changed` com `to_stage_id` e SEM
-- `to_pipeline_id`. O CHECK da tabela aceita, e a conferência de pós-voo do
-- lote também passa (ela só exige que exista ALGUM destino). O evento entra no
-- banco, aparece na aba Histórico da ficha — e **some do funil inteiro**,
-- porque `cb_funil_trajetorias` seleciona a história por `e.to_pipeline_id`.
-- É a forma exata de defeito que este projeto mais teme: dado errado, sem
-- erro, com duas telas discordando e nada ligando uma à outra.
--
-- ⚠️ A carga de 21/09 NÃO produziu nenhum: `carga.py` monta todo evento com o
-- par (etapa, funil) resolvido de `ETAPA_ID`, e a conferência de pós-voo mediu
-- **0** entre os 12.308 eventos retroativos. O que se fecha aqui é o TRAP —
-- a função continua de pé para o delta do dia do corte e para quem a chamar
-- depois, e a regra 13 do contrato diz que a coluna é obrigatória.
--
-- Só a conferência de forma muda; o resto do corpo é idêntico à 1017.
-- ============================================================
...
begin
  ...
  if v_corpo not like '%stage_changed exige to_pipeline_id%' then
    raise exception '1019: stage_changed voltou a aceitar evento sem funil — ele some das três vistas do funil';
  end if;
  if v_corpo not like '%e.to_pipeline_id is null%' then
    raise exception '1019: a conferência de pós-voo voltou a aceitar "algum destino" em vez de cobrar o funil';
  end if;
  -- o que a 1017 trouxe tem de continuar de pé
  if v_corpo not like '%to_stage_label%' or v_corpo not like '%to_stage_position%' then
    raise exception '1019: a trilha voltou a nascer muda';
  end if;
  if v_corpo not like '%d.contact_id = v_contato%' then
    raise exception '1019: o card a mover deixou de ser conferido contra o contato';
  end if;
  ...
  if has_function_privilege('anon', 'public.cb_kommo_carregar_lote(uuid, jsonb, boolean)', 'EXECUTE')
    ...
    raise exception '1019: a função ficou executável pelo navegador';
  end if;
  if not has_function_privilege('service_role', 'public.cb_kommo_carregar_lote(uuid, jsonb, boolean)', 'EXECUTE') then
    raise exception '1019: service_role perdeu o execute';
  end if;

  raise notice '1019: stage_changed sem funil agora é recusado na entrada e na saída.';
end
$conferir$;
