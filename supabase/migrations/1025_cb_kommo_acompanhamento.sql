...
--
--   P2  cb_encerrar_conversas_abertas: a folga de 2 minutos ("não encerra
--       quem acabou de falar") era conferida sobre a foto do começo do
--       SELECT que trava as linhas. A mensagem confirmada DEPOIS daquela foto
--       e ANTES de o SELECT alcançar a linha era invisível: a reabertura da
--       ingestão via a conversa ainda aberta e não esperava, e o lote a
--       encerrava com a mensagem escondida e a não lida zerada. Agora a
--       folga é conferida de novo no próprio UPDATE, que enxerga uma foto
--       tirada depois das travas, e a foto do antes sai do MESMO comando —
--       só de quem foi de fato encerrado.
--
-- ⚠️ LIMITE CONHECIDO, registrado e NÃO corrigido (o outro achado do Codex
-- no head 657b78d): o desfazer da carga (`cb_kommo_desfazer`) e o do
-- encerramento em lote leem "intocado desde a operação" como
-- `updated_at <= criado_em/encerrado_em`. `set_updated_at` grava o `now()`
-- da transação de quem escreve, que é a hora em que ela COMEÇOU: um
-- salvamento que começa milissegundos antes da transação do lote, espera a
-- trava dela e grava depois fica com `updated_at` anterior à operação, e o
-- desfazer o trataria como intocado. A janela é de um comando de
-- salvamento que já estava em voo quando o lote começou; medido na carga de
-- 21/09, nenhuma ficha nem card tem essa assinatura (updated_at na hora
-- anterior à operação). Fechar de vez é guardar a imagem da linha depois da
-- operação e comparar a linha inteira — uma coluna nova no livro e um
-- acervo das linhas que ele já tem —, e os dois desfazeres são ferramentas
...
begin
  ...
  -- encerrava a conversa com a mensagem escondida e a não lida zerada
  -- (revisão adversarial do PR #238). Esta instrução tira foto NOVA, depois
  -- de todas as linhas já travadas; e desde a trava nenhuma mensagem nova
  -- entra nelas sem esperar (o INSERT confere a FK com FOR KEY SHARE, que
  -- conflita com o nosso FOR UPDATE). Então tudo o que foi confirmado antes
  -- da trava está visível aqui.
  ...
  return jsonb_build_object(
    ...
    'poupadas_por_falar_agora', v_recentes,
    'poupadas_na_hora_de_gravar', v_tardias,
    'ms', extract(milliseconds from clock_timestamp() - v_inicio));
end;
$encerrar$;
